...
from .Types import DeathbulgeLocation
...
def create_regions(world: "DeathbulgeWorld"):
...
def create_region(world: "DeathbulgeWorld", name: str) -> Region:
    ...
    for (key, data) in location_table.items():
        if data.region == name:
            if not is_valid_location(world, key):
                continue
            location = DeathbulgeLocation(world.player, key, data.ap_code, reg)
...
def create_region_and_connect(world: "DeathbulgeWorld",
